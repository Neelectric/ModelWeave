...
for instance_idx in tqdm(range(num_instances), dynamic_ncols=True):
    ...
    instance_means_tensor = torch.tensor(instance_means).to(torch.float32) # need to convert to fp32 otherwise plt complains
...
for instance_idx, instance_means_tensor in tqdm(enumerate(cross_dataset_means), dynamic_ncols=True):
