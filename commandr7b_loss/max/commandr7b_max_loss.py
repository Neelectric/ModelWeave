...
for instance_idx in tqdm(range(num_instances), dynamic_ncols=True):
    ...
    for layer_id, layer in enumerate(model.model.layers):
        ...
        for matrix in mlp_matrices:
            ...
            max_grad = grad.max()
            layer_maxs.append(max_grad)
        instance_maxs.append(layer_maxs)
    model.zero_grad()
    # max, max and curtosis!
    ...
    instance_maxs_tensor = torch.tensor(instance_maxs).to(torch.float32) # need to convert to fp32 otherwise plt complains
    cross_dataset_maxs.append(instance_maxs_tensor)

    # Update global min and max values
    global_min = min(global_min, instance_maxs_tensor.min().item())
    global_max = max(global_max, instance_maxs_tensor.max().item())
...
cross_dataset_maxs_tensor = torch.stack(cross_dataset_maxs)
averaged_dataset_maxs = cross_dataset_maxs_tensor.max(dim=0).values
plt.figure(figsize=(12, 8))
plt.title(f"Max Gradients Heatmap of {model_id} on MATH  - Average of {num_instances}")
plt.imshow(averaged_dataset_maxs, cmap='viridis', aspect='auto')
plt.colorbar(label='Max Gradient')
plt.xlabel("Attention and MLP Matrices")
plt.ylabel("Layers")
plt.gca().invert_yaxis()  # Invert y-axis to start layers at 0 at the bottom
plt.xticks(ticks=range(7), labels=['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'])
plt.yticks(ticks=range(32), labels=range(32))
plt.savefig(os.path.join(output_dir, f"max_of_{num_instances}_heatmap_.png"))
plt.close()